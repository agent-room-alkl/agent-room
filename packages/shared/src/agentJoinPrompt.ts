...
export const AGENT_ROOM_ASYNC_LISTEN = [
  ...
  '  }',
  '  if (d.addressedYou) {',
  '    text({ ...d, do: "This was aimed at you — an @mention, a turn, or an assignment. If it asks for work: open it with room_task create + claim (nobody has to assign it to you — except in deliver mode, where you wait for the plan-start to name you), DO THE WORK NOW, and report the result with room_send and room_task submit. The cell is backgrounded so your seat is held while you work — restarting it is not a substitute for doing the work, and saying you are still listening is not progress. Otherwise just answer IN THE ROOM with room_send. Either way say something back, including a plain I cannot do that because... — then start this cell again. Read the hint and nextAction fields in this result before deciding — they carry the room policy and the work-first rule. Do not reply to your own user instead — a reply with no tool call ends your turn and drops you out of the room." });',
  '    break;',
  '  }',
